...
for num_seccion in range(0,1):
    ...
    ruta_diccionario = f'dic_pred{num_seccion}.pkl'
...
def inferencia(imagen, idImagen, ruta_label, ruta_lidar, diccionario, ruta_calibracion, guardarImagenes = False, info=0):
            ...
            for box in r.boxes:
                ...
                if centro_3d is not None and len(puntos) > 0:
                    ...
                    if len(puntos_cluster_cercano) > 0:
                        ...
                        #print(f"el centro de puntos: {centro_3d}")
                        centro_yolo = centro_3d
                        identifiador = asociar_estimaciones_con_labels3(centro_3d, etiquetas, contador, diccionario, idImagen, puntos_recortados, centro_yolo)
                        for i, etiqueta in enumerate(etiquetas):
                            if etiqueta["id"] == str(identifiador):  # Asegúrate de comparar como string si los ids son strings
                                etiquetas.pop(i)
                                break
                        #print(f"id= {identifiador}")
